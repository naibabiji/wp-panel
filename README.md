...
面板有三道锁：
- 第一道：随机生成的 8 位入口地址，像 `/26858785`，扫描器永远猜不到
- 第二道：浏览器弹窗要求输入 BasicAuth 账号密码
- 第三道：网页表单要求输入面板登录密码

三道全错的人会被自动封禁 IP。即便你只记得给面板设了强密码，其他什么都不做，服务器也是安全的。
...
- HTTP BasicAuth + Web 登录表单双重认证
- 纯 HTTPS 加密通信，不监听 HTTP 端口
- 仅对外开放 8443 一个端口

**防爆破**
- 任一认证层连续 5 次失败 → IP 自动封禁 24 小时
- Fail2ban + nftables 双机制即时生效
