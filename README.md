...
| 模块 | 说明 |
|------|------|
| **网站管理** | 一键建站（自动创建隔离用户/目录/Nginx/PHP-FPM/数据库）、暂停/启用/删除、重装 WordPress；暂停网站时同步暂缓关联的自动备份和计划任务，启用后按原计划恢复 |
...
| **密码找回保护** | 站点级三态开关（允许 / 全站禁用 / 仅禁用管理员），全站禁用同步隐藏登录页"忘记密码？"入口 |
| **数据库管理** | MariaDB 密码修改、数据库备份/恢复/上传恢复/自动备份 |
| **计划任务** | 可视化 Cron 管理、WP Cron 替代、文件增量备份、系统任务查看 |
| **文件管理器** | 上传/下载/删除/重命名/压缩/解压/剪切/复制/粘贴/多选、分片上传+断点续传；支持按名称搜索当前目录或当前目录及子目录、跨目录结果定位与下载、首页/尾页快速翻页 |
| **仪表盘** | CPU/内存/磁盘/负载实时监控、24h/7d/15d 历史趋势图 |
| **系统稳定性防护** | 低内存服务器自动创建 Swap 安全缓冲、OOM 事故告警、MariaDB/PHP-FPM/Nginx/Redis 异常退出诊断与事故资源快照 |
