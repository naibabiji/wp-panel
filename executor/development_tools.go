...
func installWPCLI(ctx context.Context) error {
	...
	req, err := http.NewRequestWithContext(ctx, http.MethodGet, wpCLIURL, nil)
	if err != nil {
		return err
	}
	client := &http.Client{Timeout: 5 * time.Minute, CheckRedirect: func(req *http.Request, via []*http.Request) error {
		if req.URL.Scheme != "https" || (req.URL.Hostname() != "github.com" && req.URL.Hostname() != "release-assets.githubusercontent.com") {
	...
	data, err := io.ReadAll(io.LimitReader(resp.Body, wpCLIMaxDownload+1))
	if err != nil {
		return err
	}
	if len(data) == 0 || len(data) > wpCLIMaxDownload {
		return errors.New("WP-CLI download size is invalid")
	}
	sum := sha512.Sum512(data)
	if hex.EncodeToString(sum[:]) != wpCLISHA512 {
