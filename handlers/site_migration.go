...
func (h *SiteMigrationHandler) Probe(c *gin.Context) {
	if h.Control == nil || h.Control.Probe(c.Request.Context(), c.Param("id")) != nil {
		siteMigrationError(c, http.StatusConflict, "common.operation_failed")
		return
	}
	addresses, _ := h.Control.DNS(c.Request.Context(), c.Param("id"))
	c.JSON(http.StatusOK, gin.H{"success": true, "dns_addresses": addresses})
}

func (h *SiteMigrationHandler) ConfirmMigration(c *gin.Context) {
	var req struct {
	...
	if h.Control == nil || strings.TrimSpace(actor) == "" || h.Control.Confirm(c.Request.Context(), c.Param("id"), req.Domain, actor, req.Reason, req.Force) != nil {
		siteMigrationError(c, http.StatusConflict, "common.operation_failed")
		return
	}
...
func (h *SiteMigrationHandler) Abandon(c *gin.Context) {
	var req struct {
		Domain         string `json:"domain"`
		Reason         string `json:"reason"`
		AllowActivated bool   `json:"allow_activated"`
	}
	if !decodeSiteMigrationJSON(c, &req) {
		return
	}
	username, _ := c.Get("session_username")
	actor, _ := username.(string)
	if h.Control == nil || strings.TrimSpace(actor) == "" || h.Control.Abandon(c.Request.Context(), c.Param("id"), req.Domain, actor, req.Reason, req.AllowActivated) != nil {
		siteMigrationError(c, http.StatusConflict, "common.operation_failed")
		return
	}
	c.JSON(http.StatusOK, gin.H{"success": true})
...
func (h *SiteMigrationHandler) MachineSourceProbe(c *gin.Context) {
	var req struct {
		PeerID          string `json:"peer_id"`
		MigrationSiteID string `json:"migration_site_id"`
	}
	...
	if h.Control == nil || h.Service == nil || h.Service.AuthorizePeer(c.Request.Context(), req.PeerID, bearerFromMigrationRequest(c)) != nil || h.Control.AuthorizeMachineTask(c.Request.Context(), req.PeerID, req.MigrationSiteID, "source") != nil {
		siteMigrationError(c, http.StatusUnauthorized, "common.operation_failed")
		return
	}
	attestation, err := h.Control.ProbeSource(c.Request.Context(), req.PeerID, req.MigrationSiteID)
	if err != nil {
		siteMigrationError(c, http.StatusConflict, "common.operation_failed")
		return
	}
	c.Header("Cache-Control", "no-store")
	c.JSON(http.StatusOK, gin.H{"success": true, "attestation": attestation})
}

func (h *SiteMigrationHandler) MachineTargetProbe(c *gin.Context) {
	var req struct {
		PeerID          string                                 `json:"peer_id"`
		MigrationSiteID string                                 `json:"migration_site_id"`
		Attestation     executor.SiteMigrationProbeAttestation `json:"attestation"`
	}
	if !decodeSiteMigrationJSON(c, &req) {
		return
	}
	...
	if h.Control.AcceptProbe(c.Request.Context(), req.MigrationSiteID, req.Attestation) != nil {
		siteMigrationError(c, http.StatusConflict, "common.operation_failed")
		return
	}
	c.Header("Cache-Control", "no-store")
...
func (h *SiteMigrationHandler) MachineTargetConfirm(c *gin.Context) {
	var req struct {
		PeerID          string `json:"peer_id"`
		MigrationSiteID string `json:"migration_site_id"`
		Domain          string `json:"domain"`
	...
	if h.Control.Confirm(c.Request.Context(), req.MigrationSiteID, req.Domain, req.Operator, req.Reason, req.Force) != nil {
		siteMigrationError(c, http.StatusConflict, "common.operation_failed")
		return
	}
	c.Header("Cache-Control", "no-store")
...
func (h *SiteMigrationHandler) MachineTargetAbandon(c *gin.Context) {
	var req struct {
		PeerID          string `json:"peer_id"`
		MigrationSiteID string `json:"migration_site_id"`
		Domain          string `json:"domain"`
	...
	if h.Control.AbandonTarget(c.Request.Context(), req.MigrationSiteID, req.Domain, req.Operator, req.Reason, req.AllowActivated) != nil {
		siteMigrationError(c, http.StatusConflict, "common.operation_failed")
		return
	}
	c.Header("Cache-Control", "no-store")
...
func (h *SiteMigrationHandler) MachineSourceAbandon(c *gin.Context) {
	var req struct {
		PeerID          string `json:"peer_id"`
		MigrationSiteID string `json:"migration_site_id"`
	}
	...
	if h.Control.AbandonSource(c.Request.Context(), req.MigrationSiteID) != nil {
		siteMigrationError(c, http.StatusConflict, "common.operation_failed")
		return
	}
	c.Header("Cache-Control", "no-store")
...
func (h *SiteMigrationHandler) SourceChunk(c *gin.Context) {
	...
	c.Header("Cache-Control", "no-store")
	c.Header("X-WP-Panel-File-SHA256", chunk.FileSHA256)
	c.Header("X-WP-Panel-Chunk-SHA256", chunk.ChunkSHA256)
	c.Header("X-WP-Panel-Chunk-Offset", strconv.FormatInt(chunk.Offset, 10))
	c.Header("X-WP-Panel-File-Size", strconv.FormatInt(chunk.TotalSize, 10))
...
func (h *SiteMigrationHandler) SourceFileShard(c *gin.Context) {
	...
	c.Header("Cache-Control", "no-store")
	c.Header("X-WP-Panel-Shard-Index", strconv.Itoa(shard.Index))
	c.Header("X-WP-Panel-Shard-Entries", strconv.Itoa(shard.EntryCount))
	c.Header("X-WP-Panel-Shard-Uncompressed-Bytes", strconv.FormatInt(shard.UncompressedBytes, 10))
