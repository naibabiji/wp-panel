...
func buildAIDevelopmentCredentialPackage(domain, host string, port int, systemUser, webRoot string, credential aiDevelopmentCredential) ([]byte, error) {
	...
	files := map[string]struct {
		...
		prefix + ".wp-panel-ai/CONNECTION.md": {content: []byte(fmt.Sprintf("# Connection details\n\n- Site: `%s`\n- Remote WebRoot: `%s`\n- SSH user: `%s`\n- SSH host: `%s`\n- SSH port: `%d`\n\nConnect through the supplied script, then read `~/WP-PANEL-AI-HANDOFF.md` on the server.\n", domain, webRoot, systemUser, host, port)), mode: 0600},
		prefix + ".wp-panel-ai/connect.sh":    {content: []byte("#!/bin/sh\nset -eu\ncredential_dir=$(CDPATH= cd -- \"$(dirname -- \"$0\")\" && pwd)\nexec ssh -F \"$credential_dir/ssh_config\" -i \"$credential_dir/id_ed25519\" wp-panel-ai \"$@\"\n"), mode: 0700},
...
1. Connect with .wp-panel-ai/connect.sh on Linux, macOS, or WSL, or .wp-panel-ai/connect.ps1 on Windows PowerShell.
